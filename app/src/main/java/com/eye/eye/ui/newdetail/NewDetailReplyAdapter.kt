...
class NewDetailReplyAdapter(
    ...
    val dataList: ArrayList<VideoReplies.Item>
        ...
        when (holder) {
            is TextCardViewHeader4ViewHolder ->{
                ...
                if (item.data.actionUrl != null && item.data.actionUrl.startsWith(Const.ActionUrl.REPLIES_HOT)){
                    ...
                    setOnClickListenter(holder.ivInto4,holder.tvTitle4){ R.string.currently_not_supported.showToast()}
                }else{
                    //相关推荐。最新评论
    ...
    private fun getTimeReply(dateMillis: Long): String {
        ...
        if (timePast > -DateUtil.MINUTE) {
            when{
                timePast < DateUtil.DAY ->{ // 采用误差一分钟以内的算法，防止客户端和服务器时间不同步导致的显示问题
                    var pastHolder = timePast / DateUtil.HOUR
                    if (pastHolder <= 0) {
                        pastHolder = 1
                    }
                    return DateUtil.getDate(dateMillis,"HH:mm")
                }
                else -> return DateUtil.getDate(dateMillis,"yyy/MM/dd")
            }
        }else{
