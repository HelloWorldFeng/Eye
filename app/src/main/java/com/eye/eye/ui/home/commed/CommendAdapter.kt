    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            is TextCardViewHeader5ViewHolder ->{
                ...
                setOnClickListenter(holder.tvTitle5,holder.ivInto5){    ActionUrlUtil.process(fragment,item.data.actionUrl,item.data.text)  }
            }
            ...
            is TextCardViewHeader8ViewHolder->{
                ...
                setOnClickListenter(holder.tvRightText8, holder.ivInto8) { ActionUrlUtil.process(fragment, item.data.actionUrl, item.data.text) }
            }
            is TextCardViewFooter2ViewHolder ->{
                holder.tvFooterRightText2.text = item.data.text
                setOnClickListenter(holder.tvFooterRightText2, holder.ivTooterInto2) { ActionUrlUtil.process(fragment, item.data.actionUrl, item.data.text) }
            }
            is TextCardViewFooter3ViewHolder ->{
                holder.tvFooterRightText3.text = item.data.text
                setOnClickListenter(holder.tvRefresh, holder.tvFooterRightText3, holder.ivTooterInto3){
            ...
            is FollowCardViewHolder ->{
                ...
                holder.tvVideoDuration.text = item.data.content.data.duration.coversionVideoDuration()
            ...
            is VideoSmallCardViewHolder ->{
                ...
                holder.itemView.setOnClickListener {
                    item.data.run {
                        NewDetailActivity.start(fragment.activity,
            ...
            is UgcSelectedCardCollectionViewHolder ->{
                ...
                holder.tvRightText.setOnClickListener {
                    EventBus.getDefault().post(SwitchPageEvent(CommendFragment::class.java))
                    EventBus.getDefault().post(RefreshEvent(CommendFragment::class.java))
                }
