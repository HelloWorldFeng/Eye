...
import com.eye.eye.ui.common.holder.UgcSelectedCardCollectionViewHolder
import com.eye.eye.ui.home.daily.DailyAdapter
import com.eye.eye.ui.ugcdetail.UgcDetailActivity
...
class CommendAdapter(val fragment: CommendFragment, val dataList: List<CommunityRecommend.Item>) :
        ...
        when (holder) {
            ...
            is FollowCardViewHolder ->{
                ...
                if (item.data.header?.iconType ?: "".trim() == "round") {
                    ...
                    holder.ivRoundAvatar.load(item.data.content.data.cover.feed,4f)
                }else{
    ...
    inner class SquareCardOfCommunityContentItemDecoration(val fragment:CommendFragment) : RecyclerView.ItemDecoration() {
        override fun getItemOffsets(
            ...
            when (position) {
                0 -> outRect.right = fragment.middleSpace
                count -> outRect.left = fragment.middleSpace
                else -> {
