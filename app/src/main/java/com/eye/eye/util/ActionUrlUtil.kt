...
object ActionUrlUtil {
    ...
    * @param acitonUrl 待处理的url
    * @param toastTitle toast提示标题or 没有匹配的事件需要处理*/
    ...
    fun process(activity: Activity, actionUrl: String?, toastTitle: String ="") {
        ...
        when {
            ...
            decodeUrl == Const.ActionUrl.RANKLIST -> {
                "${toastTitle},${GlobalUtil.getString(R.string.currently_not_supported)}".showToas()
            }
            decodeUrl.startsWith(Const.ActionUrl.TAG) -> {
                "${toastTitle},${GlobalUtil.getString(R.string.currently_not_supported)}".showToas()
            }
            decodeUrl == Const.ActionUrl.HP_SEL_TAB_TWO_NEWTAB_MINUS_THREE -> {
                EventBus.getDefault().post(SwitchPageEvent(DailyFragment::class.java))
            }
            ...
            actionUrl == Const.ActionUrl.HP_NOTIFI_TAB_ZERO -> {
                EventBus.getDefault().post(SwitchPageEvent(PushFragment::class.java))
